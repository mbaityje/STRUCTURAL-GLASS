...
class LJ:
	...
	# @jit("numba.float64(numba.object_,numba.object_)")
	def Cd_fast(self,snapA, snapB, beta):
		"""
		Provo ad accelerare Cd
		"""
		Natoms=snapA.particles.N
		assert(Natoms==snapB.particles.N)
		posA=np.array(snapA.particles.position, dtype=np.float64)
		posB=np.array(snapB.particles.position, dtype=np.float64)
		types=snapA.particles.typeid
		L=snapA.box.Lx #Assume cubic box

		output=0
		norm=np.linalg.norm
		pd=med.PeriodicDisplacement
		for k in range(Natoms):
			posAk=posA[k]
			posBk=posB[k]
			typek=types[k]
			for i in range(Natoms):
				...
				posBi=posB[i]
				typei=types[i]
				rvecA=pd(posAk,posAi,L)
				rvecB=pd(posBk,posBi,L)
				rA=norm(rvecA)
				rB=norm(rvecB)
				if typei==typek:
					if typei==0:
	...
	# Calculate EXTENSIVE Energy and Gradient of the smoothened LJ potential
	# @jit("numba.float64(numba.object_,numba.object_)")
	def CalculateEnergySlowerJit(self,snapshot):
		energia=0
		Natoms=snapshot.particles.N
		L=snapshot.box.Lx #Assume cubic box
		for i in range(Natoms):
