...
class Beylkin:
    ...
    def prony_function(self, x):
        ...
        lamda = np.log(self.gamma)
        lamda_real = np.real(lamda)
        lamda_imag = abs(np.imag(lamda))
        ...
        for ind in self.cos_inds:
            result += weights[ind] * np.exp(lamda_real[ind] * x) * np.cos(lamda_imag[ind] * x)

        # Evaluate damped sines.
        for ind in self.sin_inds:
            result += weights[ind] * np.exp(lamda_real[ind] * x) * np.sin(lamda_imag[ind] * x)
            
        # Evaluate monotonic terms.
        for ind in self.monotonic_inds:
            result += weights[ind] * np.exp(lamda_real[ind] * x)
