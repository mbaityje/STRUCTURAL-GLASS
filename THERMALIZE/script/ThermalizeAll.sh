...
do
	...
	do
		...
		case $T in
			...
			2.0)   totMDsteps=$(echo 1*10^4/$dt  |bc); thermostat=NVT; tauT=0.1; heavyTrajFreq=`echo $totMDsteps/4|bc`;   queue=gpu;   initConf=$hottestTDIR/S$isam/$thermConfName;;
			0.6)   totMDsteps=$(echo 3*10^6/$dt  |bc); thermostat=NVT; tauT=0.1; heavyTrajFreq=`echo $totMDsteps/20|bc`;  queue=gpu;   initConf=$hottestTDIR/S$isam/$thermConfName;;
			# 0.49)  totMDsteps=$(echo 1*10^8/$dt  |bc); thermostat=NVT; tauT=0.1; heavyTrajFreq=`echo $totMDsteps/40|bc`;  queue=gpu;   initConf=$hottestTDIR/S$isam/$thermConfName;;
			...
			0.49)  totMDsteps=$(echo 1*10^7/$dt  |bc); thermostat=NVT; tauT=0.1; heavyTrajFreq=`echo $totMDsteps/40|bc`;  queue=gpu; initConf=$hottestTDIR/S$isam/$thermConfName;;
			0.466) totMDsteps=$(echo 1*10^7/$dt  |bc); thermostat=NVT; tauT=0.1; heavyTrajFreq=`echo $totMDsteps/50|bc`;  queue=gpu; initConf=$hottestTDIR/S$isam/$thermConfName;;
			0.44)  totMDsteps=$(echo 1*10^7/$dt  |bc); thermostat=NVT; tauT=0.1; heavyTrajFreq=`echo $totMDsteps/100|bc`; queue=gpu; initConf=$hottestTDIR/S$isam/$thermConfName;;
			0.43)  totMDsteps=$(echo 1*10^7/$dt  |bc); thermostat=NVT; tauT=0.1; heavyTrajFreq=`echo $totMDsteps/100|bc`; queue=gpu; initConf=$hottestTDIR/S$isam/$thermConfName;;
			0.42)  totMDsteps=$(echo 1*10^7/$dt  |bc); thermostat=NVT; tauT=0.1; heavyTrajFreq=`echo $totMDsteps/100|bc`; queue=gpu; initConf=$hottestTDIR/S$isam/$thermConfName;;
		...
		time (python $exeDIR/ReadAndThermalize.py --user="$initConf -N$Natoms -s$seed -T$T -t$totMDsteps --tauT=$tauT --dt=$dt --thermostat=$thermostat --backupFreq=$backupFreq --heavyTrajFreq=$heavyTrajFreq --startfromzero=$startfromzero"  2>&1) 2>>thermalized.time
		elif [ $SYSTEM == "talapas" ]; 
		then
		nombre=N${Natoms}${PROC_TAG}T${T}i${isam}
		if [ 0 == `squeue -u$USERNAME -n $nombre|grep $USERNAME|wc -l` ]
		then
			echo sbatch --job-name=$nombre -p $queue --export=exeDIR=$exeDIR,initConf=$initConf,Natoms=$Natoms,seed="${seed}",T=$T,totMDsteps=$totMDsteps,tauT=$tauT,dt=$dt,thermostat=$thermostat,backupFreq=$backupFreq,heavyTrajFreq=$heavyTrajFreq,startfromzero=$startfromzero $scriptDIR/Thermalize.sbatch
			sbatch --job-name=$nombre -p $queue --time=$simTime --export=exeDIR=$exeDIR,initConf=$initConf,Natoms=$Natoms,seed="${seed}",T=$T,totMDsteps=$totMDsteps,tauT=$tauT,dt=$dt,thermostat=$thermostat,backupFreq=$backupFreq,heavyTrajFreq=$heavyTrajFreq,startfromzero=$startfromzero,isam=$isam $scriptDIR/Thermalize.sbatch
		fi
		else
